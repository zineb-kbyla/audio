import { quizzesQuestions } from "../src/scripts/quizzesQuestions";
...
import { IFlashcardsQueryRes as IQuizzesQueryRes } from "../src/interfaces/IFlashcardsQueryRes";
...
describe('quizzesQuestions', () => {
    let testQuiz: IQuizzesQueryRes;
    ...
    describe('API ElevenLabs', () => {
        it('should call ElevenLabs API successfully (Cas 1)', async () => {
            ...
            const params = getTTSParams(SubjectEnum.ENGLISH, testQuiz.question);
        ...
        it('should handle 401 error from ElevenLabs API (Cas 3)', async () => {
            ...
            await expect(quizzesQuestions('CP')).rejects.toMatchObject({
                response: { status: 401 },
            });
        }, 60000);
    ...
    describe('Upload sur S3', () => {
        it('should upload audio file to S3 successfully (Cas 1)', async () => {
            const audioBuffer = Buffer.from('fake-audio-data');
            const s3Key = 'audios-bewize/quizzes/questions/1.mp3';
        ...
        it('should handle S3 upload error due to large file size', async () => {
            ...
            const s3Key = 'audios-bewize/quizzes/questions/large-file.mp3';
        ...
        it('should handle S3 upload error due to full bucket', async () => {
            const audioBuffer = Buffer.from('fake-audio-data');
            const s3Key = 'audios-bewize/quizzes/questions/full-bucket.mp3';
    ...
    describe('Mise à jour de la base de données', () => {
        it('should update database with S3 URL (Cas 1)', async () => {
            const fileUrl = 'https://bewize-audios.s3.eu-west-1.amazonaws.com/audios-bewize/quizzes/questions/1.mp3';
        ...
        it('should handle SQL update error (Cas 2)', async () => {
            ...
            await expect(quizzesQuestions('CP')).rejects.toThrow('SQL update failed');
        }, 60000);
    ...
    // Additional tests for quizzesQuestions
    describe('Additional Tests for quizzesQuestions', () => {
        it('should handle no questions found for a subject', async () => {
            mockQuery.resolves({
                rows: [],
            });

            const result = await quizzesQuestions('CP');
        ...
        it('should handle database query error', async () => {
            ...
            await expect(quizzesQuestions('CP')).rejects.toThrow('Database query failed');
        });

        it('should handle empty question', async () => {
            const emptyQuiz: IQuizzesQueryRes = {
                questionid: '2',
                language: SubjectEnum.ENGLISH,
                question: '',
                answer: 'Paris',
    ...
    describe('Transaction Management', () => {
        ...
        it('should rollback transaction on invalid question', async () => {
            const invalidQuiz: IQuizzesQueryRes = {
                questionid: '3',
                language: SubjectEnum.ENGLISH,
                question: '!!!',
                answer: 'Paris',
            ...
            const params = getTTSParams(invalidQuiz.language, invalidQuiz.question);
            mockAxios.onPost(`${ELEVENLABS_API_URL}/${params.voice_id}`).reply(404);

            await expect(quizzesQuestions('CP')).rejects.toThrow('Request failed with status code 404');
        }, 60000);
    ...
    // Integration Tests for quizzesQuestions
    describe('Integration Tests for quizzesQuestions', () => {
        it('should process a valid question and update the database', async () => {
            const audioBuffer = Buffer.from('fake-audio-data');
            const s3Key = 'audios-bewize/quizzes/questions/c7c04ac8-d86e-4041-927b-191dc5850a2e.mp3';
    ...
    describe('Database Connection Failure', () => {
        it('should handle database connection failure', async () => {
            ...
            await expect(quizzesQuestions('CP')).rejects.toThrow('Database connection failed');
        });
    });
    ...
    describe('Batch Processing with BATCH_DELAY', () => {
        it('should respect BATCH_DELAY between batches', async () => {
            ...
            const numberOfBatches = Math.ceil(quizzesSet.length / CHUNK_SIZE);
            ...
            expect(result).toEqual([
                {
                    ...
                    answerid: '1',
                    questiontype: 'MULTIPLE_QCU',
                    question_audio: 'audios-bewize/quizzes/questions/04aebca4-c645-494c-b4b2-4fc18b20ee4f.mp3'
                }
            ]);
        }, 60000);
    });
    ...
    describe('Handling a large number of questions', () => {
        it('should process 1000 questions successfully', async () => {
            const quizzesSet = Array.from({ length: 2 }, (_, index) => ({
                questionid: `04aebca4-c645-494c-b4b2-4fc18b20ee4f`,
                language: SubjectEnum.ENGLISH,
            ...
            for (let i = 0; i < quizzesSet.length; i++) {
            ...
            const result = await quizzesQuestions('CP') as IQuizzesQueryRes[];
            ...
            result.forEach((item, index) => {
                ...
                expect(item.question_audio).toBe(`audios-bewize/quizzes/questions/04aebca4-c645-494c-b4b2-4fc18b20ee4f.mp3`);
            });
