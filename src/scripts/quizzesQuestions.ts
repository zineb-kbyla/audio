...
import { IFlashcardsQueryRes as IQuizzesQueryRes } from "../interfaces/IFlashcardsQueryRes";
...
export const processQuizQuestion = async (
    ...
    try {
        ...
        console.log(`🔵 Début de la transaction pour la question ${quiz.questionid}.`);
        await client.query('BEGIN');

        if (!quiz.question || quiz.question.trim() === "") {
            console.warn(`⚠️ Question vide pour l'ID : ${quiz.questionid}`);
        ...
        let sanitizedQuestion = quiz.question;

        // Remplacement des [?] par ? tout en conservant la structure
        sanitizedQuestion = sanitizedQuestion.replace(/\[(\?)\]/g, '?');

        // Nettoyage supplémentaire selon le type de question
        if (quiz.questiontype === "MULTIPLE_QCU") {
            sanitizedQuestion = sanitizedQuestion.replace(/\*/g, ".").replace(/#/g, "");
        } else {
            sanitizedQuestion = sanitizedQuestion.replace(/#/g, "");
        }

        if (sanitizedQuestion.length === 0) {
            console.warn(`⚠️ Question invalide détectée pour l'ID : ${quiz.questionid}`);
            await client.query('ROLLBACK');
            console.log("🔙 Transaction annulée (ROLLBACK) : Question invalide.");
            return null;
        }

        console.log(`⏳ Traitement de la question ${quiz.questionid}...`);
        await sleep(index * 100);

        const params = getTTSParams(quiz.language, sanitizedQuestion);
        ...
        for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
            try {
                ...
                const s3Key = `audios-bewize/quizzes/questions/${quiz.questionid}.mp3`;
...
export const quizzesQuestions = async (level: string, subject?: string) => {
    console.log(`🔍 Exécution pour le niveau : ${level}${subject ? ` et la matière : ${subject}` : ''}`);

    let query = `
        SELECT que.id AS questionid, s.title AS language, que.question, que.question_type AS questiontype
        FROM subject s
    ...
    try {
        ...
        if (quizzes.length === 0) {
            console.log("✅ Aucune question trouvée pour ces critères.");
            return [];
        }

        console.log(`🔍 Trouvé ${quizzes.length} questions sans audio.`);

        const results = [];
        for (let i = 0; i < quizzes.length; i += CHUNK_SIZE) {
            const batch = quizzes.slice(i, i + CHUNK_SIZE);
            const batchResults = await Promise.all(batch.map((quiz, index) => processQuizQuestion(quiz, index)));
